    ...
    if (response.status === 200) {
      ...
      for (let t = 0; t < tables.length; t++) { 
        ...
        document.body.appendChild(qrcode)
				setTimeout(() => {
					new QRCode(document.getElementById(tables[t].children[1].children[0].cells[1].innerHTML.replaceAll(' ', '_').replaceAll('ë', 'e').replaceAll('é', 'e').replaceAll('á', 'a').replaceAll('í', 'i').replaceAll('ö', 'o')), `https://janinezielman.github.io/schouw2023/year1#${tables[t].children[1].children[0].cells[1].innerHTML.replaceAll(' ', '_').replaceAll('ë', 'e').replaceAll('é', 'e').replaceAll('á', 'a').replaceAll('í', 'i').replaceAll('ö', 'o').slice(0, 10)}`);
				}, "1000")
      }
    }
}
